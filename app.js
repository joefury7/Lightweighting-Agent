...
function generateNXCode(pocketCount, finalMass) {
  ...
  if (state.pattern === "isogrid") {
    pyCode.push(
      ...
      '        """Build a filleted equilateral triangle using true NX arcs + straight lines.',
      '        Returns a list of NXOpen curves (3 arcs + 3 lines) forming a closed loop.',
      '        r_in = circumradius of the POCKET triangle (not the cell triangle).',
      '        r_fillet = corner fillet radius. Arc center offset from centroid = r_in - 2*r_fillet.',
      '        """',
      '        curves = []',
      '        v_angles = [(math.radians(90.0 + 120.0 * k)) if ori_sign == 1 else (math.radians(-90.0 + 120.0 * k)) for k in range(3)]',
      '        # Each fillet arc center is inset from the vertex by 2*r_fillet along the bisector',
      '        ac = [(cx + (r_in - 2.0 * r_fillet) * math.cos(a),',
      '               cy + (r_in - 2.0 * r_fillet) * math.sin(a)) for a in v_angles]',
      '        # Matrix3x3 for XY plane (used for CreateArc)',
      '        m = NXOpen.Matrix3x3()',
      ...
      '            c_nx, c_ny = ac[(k + 1) % 3]',
      '            a_curr = v_angles[k]',
      '            a_next = v_angles[(k + 1) % 3]',
      '            a_arc_s = a_curr - math.pi / 3.0',
      '            a_arc_e = a_curr + math.pi / 3.0',
      '            a_nxt_s = a_next - math.pi / 3.0',
      '            # True arc at corner k (smooth circular fillet)',
      '            arc_ctr = NXOpen.Point3d(c_x, c_y, back_z)',
      '            arc = workPart.Curves.CreateArc(arc_ctr, m, r_fillet, a_arc_s, a_arc_e)',
      '            curves.append(arc)',
      '            # Straight side from end of arc k to start of arc k+1',
      '            p_s = NXOpen.Point3d(c_x  + r_fillet * math.cos(a_arc_e), c_y  + r_fillet * math.sin(a_arc_e),  back_z)',
      '            p_e = NXOpen.Point3d(c_nx + r_fillet * math.cos(a_nxt_s),  c_ny + r_fillet * math.sin(a_nxt_s), back_z)',
      ...
      '                    if extrude_pocket_boolean(workPart, lines, revolved_body, z_direction, h_extrude, bool_sub, hp):',
      '                        pocket_count += 1',
      '                    for c in lines:',
      '                        try: c.Blank()',
      ...
      '                    hp = NXOpen.Point3d(cx2, cy2, BACK_Z)',
      '                    if extrude_pocket_boolean(workPart, lines, revolved_body, z_direction, h_extrude, bool_sub, hp):',
      '                        pocket_count += 1',
      '                    for c in lines:',
      '                        try: c.Blank()',
