...
function generateNXCode(pocketCount, finalMass) {
  ...
  const pyCode = [
    ...
    '    log(lw, "=" * 65)',
    '',
    '    # 1. PARABOLIC PROFILE SPLINE',
    '    coords = []',
    '    for i in range(51):',
    '        x = RADIUS * float(i) / 50.0',
    '        z = x * x / (2.0 * R_CURV)',
    ...
    try {
      ...
      if (goalLower.includes('90') || goalLower.includes('mass') || goalLower.includes('target') || goalLower.includes('optimize') || goalLower.includes('lightweight') || goalLower.includes('kg') || goalLower.includes('to')) {
        ...
        if (solved.achievable) {
          state.forcedMass = null;
          applyCombo(solved.faceplate, solved.cellSize, solved.ribThick);
          this.logResult(`Auto-Solve Completed! Applied ${state.pattern.toUpperCase()} Pattern: Faceplate t_f = ${state.faceplate}mm, Cell Size = ${state.cellSize}mm, Rib t_w = ${state.ribThick}mm (${solved.mass.toFixed(1)} kg).`);
        } else {
          // Unsafe target mass requested: Apply Forced Unsafe Target Parameters & Forced Target Mass
          state.forcedMass = targetMass;
          applyCombo(solved.unsafeFaceplate, solved.unsafeCellSize, solved.unsafeRibThick);
          this.logResult(`FORCED TARGET (${targetMass.toFixed(1)} kg) on ${state.pattern.toUpperCase()}: Applied Faceplate t_f = ${state.faceplate}mm, Cell Size = ${state.cellSize}mm, Rib t_w = ${state.ribThick}mm (Structural Warning Active).`);
        }
      }
