...
function generateNXCode(pocketCount, finalMass) {
  ...
  if (state.pattern === "isogrid") {
    pyCode.push(
      ...
      '        """Build filleted equilateral triangle: 3 true arcs + 3 straight lines."""',
      '        curves = []',
      ...
      '        # CreateArc overload: (Point3d center, Vector3d xDir, Vector3d axisDir, float radius, float startAngle, float endAngle)',
      '        x_dir  = NXOpen.Vector3d(1.0, 0.0, 0.0)  # X axis in plane',
      '        ax_dir = NXOpen.Vector3d(0.0, 0.0, 1.0)  # Z normal — arc lies in XY plane',
      '        for k in range(3):',
      '            c_x, c_y = ac[k]',
      '            c_nx, c_ny = ac[(k + 1) % 3]',
      '            a_curr = v_angles[k]',
      ...
      '            # True smooth arc at corner k',
      '            arc_ctr = NXOpen.Point3d(c_x, c_y, back_z)',
      '            arc = workPart.Curves.CreateArc(arc_ctr, x_dir, ax_dir, r_fillet, a_arc_s, a_arc_e)',
      '            curves.append(arc)',
      '            # Straight side tangent to arc k and arc k+1',
