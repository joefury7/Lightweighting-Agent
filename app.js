...
function initEventListeners() {
  ...
  document.querySelectorAll('[data-pattern]').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      state.pattern = target.getAttribute('data-pattern');
      updateCalculation();
    });
  });
...
function generateNXCode(pocketCount, finalMass) {
  ...
  const pyCode = [
    '# ================================================================================',
    '# NX OPEN PYTHON JOURNAL: YODER VOL 2 AGGRESSIVE ISOGRID LIGHTWEIGHT MIRROR',
    '# ================================================================================',
    ...
    '# Technique:    Flat-back adaptive-depth Isogrid pocketing with RightHandSide expressions',
    '# ================================================================================',
    'import NXOpen',
    ...
    '    section1.AddToSection([curves_rule], spline_curve, NXOpen.NXObject.Null, NXOpen.NXObject.Null, NXOpen.Point3d(RADIUS/2.0, 0.0, 0.0), NXOpen.Section.Mode.Create, False)',
    '',
  ...
  if (state.pattern === "isogrid") {
    pyCode.push(
      ...
      '        denom = R_CURV * (1.0 + math.sqrt(max(0.0001, 1.0 - (1.0 + CONIC_CONSTANT) * (r**2) / (R_CURV**2))))',
      '        z_front = (r**2) / denom',
      '        return max(10.0, z_front - FACESHEET - BACK_Z)',
      '',
      '    def build_filleted_triangle(cx, cy, r_in, r_fillet, ori_sign, back_z, rot_angle=0.0):',
      '        """Filleted equilateral triangle at (cx,cy), rotated by rot_angle rad in XY plane.',
      '        Returns list of curves (arcs or line-segment approximations) + 3 straight edges."""',
      '        curves = []',
      ...
      '        ax_dir = NXOpen.Vector3d(0.0, 0.0, 1.0)',
      '        for k in range(3):',
      '            c_x, c_y = ac[k]',
      '            c_nx, c_ny = ac[(k + 1) % 3]',
      '            a_curr = v_ang[k]',
      ...
      '            # Corner fillet arc — try true CreateArc, fallback to 16-seg lines',
      '            arc_ctr = NXOpen.Point3d(c_x, c_y, back_z)',
      '            try:',
      '                arc = workPart.Curves.CreateArc(arc_ctr, x_dir, ax_dir, r_fillet, a_s, a_e)',
      '                curves.append(arc)',
      '            except Exception:',
      '                n_sub = 16  # 16-segment line approximation (smooth at any scale)',
      ...
      '            # Straight side from arc k end to arc k+1 start',
      ...
      '',
      '    # ── SECTOR APPROACH ──────────────────────────────────────────────────────',
      '    # Step 1: Compute pocket centroids in the canonical 0°–60° sector.',
      '    #         Using modular sector assignment so each pocket belongs to exactly',
      '    '  +  '    #         one sector — no duplicates at sector boundaries.',
      '    sector_pockets = []  # list of (bx, by, ori)',
      '    sector_span = math.pi / 3.0  # 60°',
      '    for j in range(-n_rows, n_rows + 1):',
      '        y_base = j * row_h',
      ...
      '            ]:',
      '                d = math.hypot(bx, by)',
      '                if d < 1e-6 or d > max_r or d < CENTRAL_EXCLUDE_R:',
      '                    continue',
      '                # Canonical sector: map angle to [0, 60°) so each point in one sector only',
      '                ang = math.atan2(by, bx)',
      '                if ang < 0: ang += 2.0 * math.pi',
      '                ang_in_sector = ang % sector_span',
      '                if ang_in_sector < sector_span - 1e-6:',
      '                    sector_pockets.append((bx, by, ori))',
      '',
      '    log(lw, f"  Sector pockets (0°–60°): {len(sector_pockets)}")',
      '',
      '    # Step 2: Generate all 6 rotated copies of the sector.',
      '    #         Boundary pockets extend slightly past outer/inner rim — NX body clips them.',
      '    for sector_idx in range(6):',
      '        theta = sector_idx * sector_span',
      '        cos_t = math.cos(theta)',
      '        sin_t = math.sin(theta)',
      '        for (bx, by, ori) in sector_pockets:',
      '            cx = bx * cos_t - by * sin_t   # rotate centroid',
      '            cy = bx * sin_t + by * cos_t',
      '            if is_close_to_support(cx, cy, hubs_list, hub_outer_limit):',
      '                continue',
      '            curves = build_filleted_triangle(cx, cy, r_in, r_fillet, ori, BACK_Z, theta)',
