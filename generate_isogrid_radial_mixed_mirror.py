# ==============================================================================
# SIEMENS NX OPEN PYTHON JOURNAL: ISOGRID + RADIAL MIXED LIGHTWEIGHT MIRROR
# ==============================================================================
# Pattern Architecture: ISOGRID CORE + RADIAL OUTER RIM HYBRID
#   - Core Region  (r <= 400 mm): Isogrid Triangular Pockets (JWST / Space Optics)
...
#   3. Select this script (generate_isogrid_radial_mixed_mirror.py) and click "Run".
# ==============================================================================
...
def tri_verts(cx, cy, side, ori, z):
    """Calculates 3 vertices of an equilateral triangle pocket."""
    r = side / math.sqrt(3.0)
    verts = []
    for k in range(3):
        a = math.radians(90.0 + 120.0 * k) if ori == 1 else math.radians(-90.0 + 120.0 * k)
...
def main():
    ...
    CELL_SIDE = 150.0          # Inner Isogrid cell side S (mm)
    TRANSITION_RADIUS = 400.0  # Boundary radius between Isogrid core & Radial outer (mm)
    ...
    lw.WriteLine("=" * 75)
    lw.WriteLine("   SIEMENS NX ISOGRID + RADIAL MIXED-PATTERN MIRROR GENERATOR")
    lw.WriteLine("=" * 75)
    ...
    lw.WriteLine(f" Inner Core Pattern    : Isogrid Triangular (0 to {TRANSITION_RADIUS} mm)")
    ...
    set_expression(workPart, "CELL_SIDE", CELL_SIDE)
    ...
    lw.WriteLine("   -> Parabolic solid blank created successfully.")

    # ==========================================================================
    # 2. SUBTRACT INNER ISOGRID TRIANGULAR POCKETS (r <= TRANSITION_RADIUS)
    # ==========================================================================
    lw.WriteLine("\n[Step 2/4] Subtracting Inner Isogrid Triangular Pockets...")
    ...
    n_rows = int(TRANSITION_RADIUS / row_h) + 2
    n_cols = int(TRANSITION_RADIUS / CELL_SIDE) + 2
    ...
    for j in range(-n_rows, n_rows + 1):
        ...
        for i in range(-n_cols, n_cols + 1):
            ...
            cy = y_base + row_h / 3.0
            if math.hypot(cx, cy) + pocket_radius <= (TRANSITION_RADIUS - RIB_THICK / 2.0):
                verts = tri_verts(cx, cy, pocket_side, 1, BACK_Z)
                lines = []
                for k in range(3):
                    p1 = NXOpen.Point3d(verts[k][0], verts[k][1], BACK_Z)
                    p2 = NXOpen.Point3d(verts[(k+1)%3][0], verts[(k+1)%3][1], BACK_Z)
            ...
            if math.hypot(cx2, cy2) + pocket_radius <= (TRANSITION_RADIUS - RIB_THICK / 2.0):
                verts2 = tri_verts(cx2, cy2, pocket_side, -1, BACK_Z)
                lines2 = []
                for k in range(3):
                    p1 = NXOpen.Point3d(verts2[k][0], verts2[k][1], BACK_Z)
                    p2 = NXOpen.Point3d(verts2[(k+1)%3][0], verts2[(k+1)%3][1], BACK_Z)
                    lines2.append(workPart.Curves.CreateLine(p1, p2))
                if extrude_subtract_pocket(workPart, lines2, blank_body, z_direction, POCKET_DEPTH, bool_sub):
                    iso_pocket_count += 1

    lw.WriteLine(f"   -> Subtracted {iso_pocket_count} inner isogrid triangular pockets.")
    ...
    for ring_idx in range(n_radial_rings):
        ...
        n_spokes = max(12, int(round(circumference / CELL_SIDE)))
    ...
    total_pockets = iso_pocket_count + radial_pocket_count
    ...
    lw.WriteLine(f"  Total Pockets Subtracted: {total_pockets} ({iso_pocket_count} Isogrid + {radial_pocket_count} Radial)")
    lw.WriteLine(f"  Faceplate Safety Margin : {FACEPLATE_THICK} mm solid front skin")
    lw.WriteLine("=" * 75)
    lw.WriteLine(" SUCCESS: Isogrid + Radial Mixed Parabolic Mirror Generated in Siemens NX!\n")
